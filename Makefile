...
# Qwen 3.8 is the supported path for 16 GB Apple Silicon machines.  These
# targets are intentionally independent from the legacy C/Metal build below.
QWEN_CLI ?= ./dwarfstar
QWEN_SETUP_ARGS ?=
QWEN_DOWNLOAD_ARGS ?=
QWEN_DOCTOR_ARGS ?=
...
CORE_OBJS = ds4.o ds4_distributed.o ds4_tp.o ds4_ssd.o ds4_metal.o ds4_layer_pack.o ds4_qwen.o ds4_qwen_quants.o
CPU_CORE_OBJS = ds4_cpu.o ds4_distributed.o ds4_tp.o ds4_ssd.o ds4_layer_pack.o ds4_qwen.o ds4_qwen_quants.o
else
...
CORE_OBJS = ds4.o ds4_distributed.o ds4_tp.o ds4_ssd.o ds4_cuda.o ds4_layer_pack.o ds4_qwen.o ds4_qwen_quants.o $(MMQ_OBJS)
CPU_CORE_OBJS = ds4_cpu.o ds4_distributed.o ds4_tp.o ds4_ssd.o ds4_layer_pack.o ds4_qwen.o ds4_qwen_quants.o
...
.PHONY: qwen-help qwen-setup qwen-download qwen-doctor qwen-chat qwen-server qwen-bench qwen-test

qwen-help:
	@echo "Qwen 3.8 27B on Apple Silicon (primary 16 GB path):"
	@echo "  make qwen-setup                 Create .venv and install pinned MLX dependencies"
	@echo "  make qwen-download              Download the pinned 3-bit target and optional MTP head"
	@echo "  make qwen-doctor                Check Apple Silicon, MLX and the local model cache"
	@echo "  make qwen-chat                  Start an interactive chat (serial unless benchmark selects MTP)"
	...
	@echo "  make qwen-chat QWEN_CHAT_ARGS='--no-mtp --ctx-size 1024'"
	...
	@echo ""
	@echo "Legacy C/Metal, CUDA and ROCm targets remain available through make help."
...
help:
	@echo "Primary Qwen 3.8 path for 16 GB Apple Silicon: make qwen-help"
	@echo ""
	@echo "Legacy native DS4 build targets:"
	@echo "  make all          Build Metal ./ds4, ./ds4-server, ./ds4-bench, ./ds4-eval, and ./ds4-agent"
...
help:
	@echo "Qwen 3.8 requires Apple Silicon: run make qwen-help on a supported Mac"
	@echo ""
	@echo "Legacy native DS4 build targets:"
...
ds4_qwen.o: ds4_qwen.c ds4_qwen.h ds4_qwen_quants.h ds4.h
	$(CC) $(CFLAGS) -c -o $@ ds4_qwen.c

ds4_qwen_quants.o: ds4_qwen_quants.c ds4_qwen_quants.h
ifeq ($(UNAME_S),Darwin)
	$(CC) $(CFLAGS) -fblocks -c -o $@ ds4_qwen_quants.c
else
	$(CC) $(CFLAGS) -c -o $@ ds4_qwen_quants.c
endif

ds4_ssd.o: ds4_ssd.c ds4_ssd.h
	$(CC) $(CFLAGS) -c -o $@ ds4_ssd.c

ds4_cli.o: ds4_cli.c ds4.h ds4_ssd.h ds4_distributed.h ds4_help.h linenoise.h ds4_qwen.h
...
tests/test_engine_mgpu_placement: tests/test_engine_mgpu_placement.o ds4_cpu_test_hooks.o ds4_distributed.o ds4_tp.o ds4_ssd.o ds4_layer_pack.o ds4_qwen.o ds4_qwen_quants.o
	$(CC) $(CFLAGS) -o $@ $^ $(METAL_LDLIBS)
...
test: ds4_test ds4_agent_test ds4-eval q4k-dot-test mxfp4-dot-test \
	...
	@if [ -f "$(DS4_TEST_MODEL)" ]; then \
		DS4_TEST_MODEL="$(DS4_TEST_MODEL)" ./ds4_test; \
	else \
		echo "legacy model tests: skipped, missing $(DS4_TEST_MODEL)"; \
	fi
