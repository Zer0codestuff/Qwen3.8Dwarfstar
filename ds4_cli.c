...
int main(int argc, char **argv) {
    ...
    if (ds4_gguf_is_qwen35(cfg.engine.model_path)) {
        if (!cfg.gen.ctx_size_set) cfg.gen.ctx_size = 512;
        if (!cfg.gen.think_mode_set) cfg.gen.think_mode = DS4_THINK_NONE;
    }
    ...
    if (!cfg.inspect && !ds4_engine_is_qwen35(engine)) {
        log_context_memory(cfg.engine.backend,
                           cfg.gen.ctx_size,
                           ds4_engine_prefill_chunk(engine),
                           cfg.engine.ssd_streaming);
